...
class TestAlignPartsRewriteGuard(unittest.TestCase):
    """B 方案（2026-09-20 用户批准）：对齐只许**修衔接**，并拦住重复/漏译/长度暴涨/悬空开头。

    用户原话："就拿'作为自由原型师约6年，一直从事手办造型工作。'为例，翻译的没错啊，只是不够好"，
    同时要求"按你这方案，重复这个问题应该也不会再发了"；随后又纠正过一次措辞（同日）：
    **不许要求"每行自足"** —— "翻译字幕往往几句连起来看才是完整的句子"，要求每行成句只会诱发
    跨行重复与凭空增补。因此这里钉住三件事：
      * 允许的是"移动边界/虚词、最多补一个虚词"这类最小调整（裸从句仍可以是好切分）；
      * 重复照样拦：原文已有的说法在拼接里变多（历史事故"同时"），以及**原文没有、拼接里却出现
        两次**的说法（`同时同时也…`，旧规则按原文计数会漏掉它）；
      * 不许以悬空助词开头（`…原型 | 的软件`），且这条在两档严格度下都生效。
    """
