...
| 8 | `st_components/sidebar_setting.py` | `subtitle_length_controls`（2026-09-20 从主区移入）+ `polish_controls`（2026-09-21 新增，紧跟其后） | 侧边栏「✂️ 字幕长度调节」/「✨ 字幕润色」 |
...
| `subtitle_length_controls` | `st_components/sidebar_setting.py`（2026-09-20 从 `st.py` 主区**移到侧边栏**） | 字幕长度面板 expander「✂️ 字幕长度调节」 | ① 开关「按语言自动设置」（`help`：切语言即按推荐档位覆盖下面两个值，手改无效）→ `subtitle.auto_length_by_language`，打开时立刻按当前语言下发一次档位（2026-09-21 `dbd7a4f` 起不再弹 toast）并 `st.rerun`；② 两个 `st.number_input`「首次粗切词数上限」/「单行最大字符数」（标题里不再带键名，单位与 config 键 `max_split_length` / `subtitle.max_length` 移进各自的 `help`）：自动模式下都**置灰**，"仅转录+关断句"时第一项也置灰并多一行 caption「ℹ️ 仅转录 + 关闭 LLM 断句：本项不参与」（**2026-09-21 `6b2956e` 撤掉常驻的 `📐 <档位说明>` caption、`dbd7a4f` 又去掉开关的 toast 后，面板自身已不显示档位说明，这行是唯一的 `st.caption`；档位说明现在只在切语言的 toast 里出现，见 `st_components/sidebar_setting.py::sync_subtitle_lengths`**）；③「保存手填值」（自动模式禁用）与「恢复当前语言推荐值」（`apply_language_profile(force=True)`，开关关着也能一键套用）；宽度用官方新写法 `width="stretch"` |
| `polish_controls` | `st_components/sidebar_setting.py`（2026-09-21 新增，紧跟「✂️ 字幕长度调节」之后） | 字幕润色面板 expander「✨ 字幕润色」 | **三个** `st.toggle`（做不做 → 是否允许思考 → 润色范围）：① `翻译后润色字幕措辞` → `subtitle.polish_translation`（默认关）；② `允许模型思考` → `subtitle.polish_thinking`（默认开）；③ `只润色有分句的长行` → `subtitle.polish_long_lines_only`（默认关）。②③ 在 ① 关闭时 `disabled` 置灰；每个 toggle 改动即 `update_key` + `st.rerun(scope="app")`（`load_key_or` 读默认值）；三个 `help` 各一行（①"额外调用 LLM 逐行润色：只改措辞、不改信息"；②"关掉节约大量 token，但更容易丢词（回退的行更多）。"；③"短句与无分句的行原样保留。"）——面板内**没有** caption（末尾那条"每 20 行一次调用…"已于 2026-09-21 按用户要求删除，见 `dbd7a4f` 之后的同批文案精简）；**token 数字与「审校不受影响」这句也已从 UI 撤掉（`6b2956e`），只在代码注释与 devdocs 里**；"行数不变""仅转录模式跳过"仍是代码行为，只是不再进文案。打开 ① 后 step5.2 才会真的调 LLM |
...
| `subtitle.polish_translation` | `st_components/sidebar_setting.py`（侧边栏「✨ 字幕润色」） | step5.2 字幕润色的开关（**2026-09-21 新增，默认 `false`**）：打开后 step5 与 step6 之间多跑一步 LLM 润色（每 20 行一次调用 + 每个有改动的批次一次审校）；关着时该步零调用，step6 立刻回到未润色的字幕表 |
| `subtitle.polish_thinking` / `subtitle.polish_long_lines_only` | 同一个面板里的后两个 toggle（**2026-09-21 新增**，仅在 `subtitle.polish_translation` 打开时可用） | 决定这一步的**花费与范围**：`polish_thinking` 默认 `true`，关掉则**润色调用**带 `extra_body={"thinking": {"type": "disabled"}}`（省约 91% token，只算润色那部分）、覆盖率门槛自动收紧到 0.85；**审校调用 `_audit_info_changes(pairs, stats)` 没有 `extra_body` 形参，不跟随这个开关**（恒按模型默认档思考开、约 860 tokens/次），所以换缓存分区的只有润色（`polish_subs[_nothink]`）；`polish_long_lines_only` 默认 `false`，打开则只送"有分句的长行"、其余行原样保留（见 [`../02-pipeline/05-字幕切分与时间轴.md`](../02-pipeline/05-字幕切分与时间轴.md) §5.2、§7.8） |
