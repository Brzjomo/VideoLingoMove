...
| `batch/utils/gui.py` | 428 | 批量模式**唯一入口**：页面布局、设置侧边栏、文件夹选择、两个操作按钮、状态显示 |
...
| 4 | `python -m streamlit run "batch\utils\gui.py"` | 用 Streamlit 跑 GUI（**不是 tkinter**）；脚本以 `__main__` 执行，因此 `batch/utils/gui.py:427` 的 `if __name__ == "__main__": main()` 生效 |
...
1. **cwd 假设链**：`StartBatch.bat:3` 的 `cd ..` 依赖「cwd = `batch/`」；若从别的目录调用批处理脚本，启动阶段 cwd 会不对。真正兜底的是 `batch/utils/gui.py:411` 的 `os.chdir(root_dir)`，但它只在点击「开始批量处理」之后执行——启动阶段的 `batch/utils/gui.py:24 CONFIG_PATH` 已是绝对路径，所以配置读取是安全的。
