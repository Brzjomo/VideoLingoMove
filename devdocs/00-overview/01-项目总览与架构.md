...
VideoLingo 是一个「视频翻译 + 字幕生成」工具：输入一个视频（或 YouTube 链接），输出带原语言/目标语言字幕的视频。核心卖点是 **词级时间轴对齐 + LLM 按句意重切分 + Netflix 标准单行字幕**。
...
| 层次 | 技术 | 用途 | 代码位置 |
| --- | --- | --- | --- |
| 交互层 | Streamlit | 单页 Web UI，同步阻塞式执行 | `st.py`、`st_components/` |
...
| 音频/视频 | FFmpeg（根目录自带 `ffmpeg.exe`）、librosa | 转码、切分、压制 | 各 `core/step*.py` |
...
| 路径 | 职责 | 是否可删/可换 |
| --- | --- | --- |
| `st.py` | Web 主入口，编排字幕流水线 | 核心 |
| `st_components/` | Streamlit 侧边栏设置、下载区块、通用工具与 CSS | 核心（替换 UI 时可整体替换） |
| `core/` | 全部流水线逻辑，`stepN_*.py` 的 N 即执行顺序 | 核心 |
| `core/all_whisper_methods/` | ASR 适配层（WhisperX / 火山）、Demucs、TOS 对象存储 | 可增删引擎 |
...
| `output/` | **运行时工作目录**：下载的视频、音频、日志、字幕、成片 | 可删（下次重新跑） |
...
| 路径 | 入口 | 并发 | 状态 | 典型用途 |
| --- | --- | --- | --- | --- |
| 交互式 | `st.py` | 单视频串行 + 内部线程池 | `st.session_state` 无持久化，靠文件存在性判断阶段完成 | 调试、单条视频 |
| 批处理 | `batch/`（**第二个 Streamlit 应用**） | 多视频串行，**无线程** | 任务表 `batch/tasks_setting.xlsx` | 无人值守、夜间跑量 |
| 单步脚本 | `python -m core.stepN_xxx` | 单步 | 无 | 定位问题、重跑某一步 |

> ⚠️ **两种模式共用同一个工作目录 `output/`，不能同时使用**。批量模式处理每个视频前会 `rmtree` 项目根的 `output/`（`batch/utils/video_processor.py:27`），会把单次模式的中间产物直接删掉。
...
5. **人工介入点**：`pause_before_translate` 允许在术语提取后暂停，由用户在页面上确认术语再继续（UI 两段式，非 `input()`）。
...
**字幕链路**：下载视频 → 抽音频 → （可选 Demucs）→ ASR 词级转写 `cleaned_chunks.xlsx` → spaCy+LLM 按句意重切 → LLM 总结术语 → LLM 三步翻译 → 按字幕长度限制再切 → 词级时间戳回填生成 SRT → 压制到视频 `output_sub.mp4`
...
- **换掉 Streamlit**：把 `st.py` 的两个编排函数替换为 CLI/服务即可，`core/` 不依赖 streamlit（`core/` 内无 streamlit import）。
