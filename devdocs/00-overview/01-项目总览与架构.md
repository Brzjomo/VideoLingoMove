...
当前代码版本：`config.yaml: version: "3.0.4"`（本项目是上游 VideoLingo 的二次开发分支，见文末「与上游的差异」）。
...
| `tests/` | 标准库 `unittest` 回归测试（12 个文件 / 242 例，多数无需重依赖） | 测试 |
...
> ⚠️ **两种模式共用同一个工作目录 `output/`，不能同时使用**。批量模式处理每个视频前会清空项目根的 `output/`（`batch/utils/video_processor.py` 调用 `prepare_output_folder`，真正的 `shutil.rmtree` 在同文件），会把单次模式的中间产物直接删掉。这也是转录缓存刻意放在 `output/` 之外（`.cache/asr/`）的原因之一。
...
flowchart LR
 ...
 S52["step5_2_polish_subs（可选，默认关）"]
...
| `core/config_utils.get_source_language` | `prompts_storage`（5 处）、`spacy_utils/load_nlp_model.py`、`spacy_utils/split_by_mark.py`、`spacy_utils/split_long_by_root.py`、`step3_2`、`step5` | 源语言的**唯一判定点**：以 `whisper.language` 为准，仅 `auto` 时回退 `whisper.detected_language`，都不可用则 `ValueError` |
| `core/step1_ytdlp.find_media_file` | `step2`、`step7`、`onekeycleanup`、`st_components/download_video_section.py` | 定位源媒体并区分音频/视频（先读 `output/input_manifest.json`，没有才回退扩展名扫描） |
| `st_components/task_runner.TaskRunner.check_cancel` | 经 `easy_util.check_cancel`（`easy_util.py`）被 `step2` 分段循环等长循环调用 | 暂停时阻塞、停止时抛 `StopTask`；没有活跃 runner 时是空操作 |
...
5. **人工介入点**：`pause_before_translate` 允许在术语提取后暂停，由用户在页面上确认术语再继续。暂停由 `st_components/task_runner.py: TaskRunner.request_review_pause`从 worker 线程发起，UI 按 `paused_for_review` 标记渲染确认界面（两段式，非 `input`）。
...
8. **协作式取消**：长循环在检查点调用 `easy_util.check_cancel`（`easy_util.py`）；它惰性导入 `TaskRunner`，因此 `core/` 不反向依赖 Streamlit，命令行单跑时是空操作。
...
> 📌 输入类型由 `core/step1_ytdlp.find_media_file`判定：优先读 `output/input_manifest.json`，没有清单才按扩展名回退。上传音频不再被包成 `black_screen.mp4`。
...
| **移植上游 3.0.4 的可移植改动**（源语言统一解析、转录缓存、任务暂停/停止、`installer.py`/`launch.py` 等） | 见 ../05-guides/上游合并记录 |
...
- ../05-guides/上游合并记录 — 本次合并移植了什么、拒绝了什么
- [../`05-guides/07-症状速查.md`](../05-guides/07-症状速查.md) — 合并引入的行为变化对应的症状表
