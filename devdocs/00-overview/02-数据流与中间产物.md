...
flowchart TD
 ...
 TRS -->|"step5.2 polish_subs_main（subtitle.polish_translation=true 时）"| TRP["output/log/translation_results_polished.xlsx"]
 TRS --> FMT["step6 align_timestamp（pick_translation_file 选表）"]
 TRP --> FMT
 CC --> FMT
...
| `output/audio/enhanced_vocals.mp3` / `.wav` | `core/step2_whisperX.py: enhance_vocals` | ASR（人声按实测电平归一到 −20 dBFS 后送识别，增益由 `compute_normalization_gain` 峰值受限） | mp3（whisper）/ 16 kHz WAV（volcano） | — |

> 📌 `output/black_screen.mp4` 是**历史产物**：早期「上传音频」会把音频用 ffmpeg 包成 640×360 黑底视频再走完整流程，现已改为写 `output/input_manifest.json`（`st_components/download_video_section.py`）。
...
| `output/log/translation_results_for_subtitles.xlsx` | `step5_splitforsub.split_for_sub_main` | `step5.2`（可选）、`step6` | 列：`Source`、`Translation` | 无 |
| `output/log/translation_results_polished.xlsx` | `step5_2_polish_subs.polish_subs_main`（**仅开关打开时生成**） | `step6`（仅当开关打开 **且** 行数与上一行相等时） | 列：`Source`、`Translation`；**行数与 `translation_results_for_subtitles.xlsx` 恒等**（`Source` 原样复制，只替换译文措辞） | 无（重跑即覆盖） |
...
| `output/output_sub.mp4` | `step7 merge_subtitles_to_video` | 硬字幕成片（Burn-in 开启且输入为视频时）；**输入为音频时不产出** | 无 |
| `output/log/subtitle_stage_done.txt` | `step7 _mark_stage_done` | **UI 判定「字幕阶段完成」的开关**（不再用 `output_sub.mp4` 是否存在，因为 Burn-in 关闭或音频输入时都不产出成片） | 无 |

> 📌 **输入类型决定 `step7` 的行为**（由 `core/step1_ytdlp.find_media_file` 返回的 `media_type` 区分，清单优先、扩展名兜底）：
> - **音频输入**（`input_manifest.json` 里 `type: "audio"`）→ 在压制之前就返回（`core/step7_merge_sub_to_vid.py`），只写完成标记、只交字幕文件
...
> ⚠️ **缓存陷阱**：`output/gpt_log/<log_title>.json` 同时承担"缓存"角色。修改 `core/prompts_storage.py` 里的提示词后，**必须删除对应 log_title 的 json**（如 `summary.json`、`sentence_splitbymeaning.json`、`translation.json`、`align_subs.json`、`subtitle_trim.json`、`polish_subs.json`、`polish_audit.json`），否则会静默复用旧结果。其中 step5.2 的**润色**分区另有一个 `_nothink` 变体（`polish_subs_nothink.json`，`subtitle.polish_thinking=false` 时用），改提示词时要连它一起删；**审校只有一个分区 `polish_audit.json`**（审校不跟随思考开关，恒按模型默认档跑）。见 [`../03-subsystems/01-LLM调用与提示词.md`](../03-subsystems/01-LLM调用与提示词.md)。
...
- **单位**：秒（float）。
- **清洗规则**：跳过纯空白词；跳过长度 > 20 字符的词；去掉法文 `»`/`«`；缺失时间戳的词用「前一个词的 end」或「后一个有时间戳的词的 start」补齐（`whisperX_utils.py`）。
...
匹配算法要点（`core/step6_generate_final_timeline.py`）：把所有词 `remove_punctuation(word.lower)` 后**无缝拼接**成长串，并记录每个字符位置对应的词下标；对每个句子同样清洗后**去掉空格**，在长串上滑动精确匹配。匹配失败直接 `raise ValueError`（无回退）。
...
由 `generate_subtitle_string`（`step6_generate_final_timeline.py`）统一生成：
...
由 `core/all_whisper_methods/transcription_cache.py` 维护，`core/step2_whisperX.py: transcribe`读写：

- **目录**：仓库根的 `.cache/asr/<key>/`，**在 `output/` 之外**，故意不受 `cleanup` 影响（避免"归档一下就重复付费识别"）。已被 `.gitignore` 忽略。
- **键 `key`**：`cache_key`= `sha256(json)`，身份含 `schema`、源媒体**内容** md5、`whisperx`/`faster-whisper`/`demucs` 包版本，以及 `_asr_cache_settings`（`core/step2_whisperX.py`）给出的 `asr_engine` / `model` / `language` / `demucs`；`asr_engine=volcano` 时再加 10 个 `volcano_asr.*` 参数。**不含**密钥、文件名与翻译设置。
- **条目 `part`**：`complete`（整条媒体的合并结果）或 `f"{start:.2f}_{end:.2f}"`（分段结果）。
- **命中行为**：`complete` 命中 → `transcribe` 直接 `process_transcription` + `save_results` 返回，**连 Demucs 人声分离都不跑**；只有部分分段命中 → 在分段循环里逐段复用，只补缺失的段。
- **失效/清理**：换模型、换引擎、改识别语言、换源文件内容会自动失效（`whisper.cache: false` 可整体关闭）；手工清理用页面上的「♻️ 转录缓存」按钮或 `transcription_cache.clear_cache`。
...
| `logs/` | `launch.py: main`（处 `LOG_DIR.mkdir(exist_ok=True)`） | 手动；每次启动新写一个 `videolingo_<时间戳>.log`（文件名由 `log_path`按时间戳生成），不会被覆盖 |
...
| 想只跑一半流程 | 设置 | 效果 |
| --- | --- | --- |
| 只要转录，不翻译 | `transcription_only: true` | `step4_2` 直通：`Translation = Source`；`step4_1` 仍会被 `st.py` 调用？**不会**——`st.py: build_task_steps`在直通分支里只建一个空的 `terminology.json`，不把 `step4_1` 加进步骤表 |
...
| 润色（step5.2）少花 token | `subtitle.polish_long_lines_only: true`（只润色有分句的长行，预筛跳过的行原样保留）+ `subtitle.polish_thinking: false`（关思考，**润色那一批**省约 91%/批，覆盖率门槛自动收紧到 0.85、回退更多行；审校不随开关变化，仍按默认档 ≈ 860 tokens/次）；侧边栏「✨ 字幕润色」面板里的后两个 toggle 写的就是这两个键 | 实测某 79 行片子：全量 + 开思考 ≈ +30% 全流程 token，预筛 + 关思考 ≈ +5%；开关只改**调用参数与预筛**，不改产物路径与列（`translation_results_polished.xlsx` 仍与 step5 产物逐行等长），详见 [`../02-pipeline/05-字幕切分与时间轴.md`](../02-pipeline/05-字幕切分与时间轴.md) §7.8、§九 |
...
- ../05-guides/上游合并记录 — `input_manifest.json` 与 `.cache/asr/` 的来源
- [../`05-guides/07-症状速查.md`](../05-guides/07-症状速查.md) — 缓存三处位置（最容易混）
