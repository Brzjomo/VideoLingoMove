...
> `core/step10_gen_audio.py`、`core/step11_merge_full_audio.py`、`core/delete_retry_dubbing.py`、
> `core/all_tts_functions/*`（含 `tts_main.py`）。
>
...
| `batch/utils/video_processor.py` | 326 | 批处理编排：`dubbing_steps`（`batch/utils/video_processor.py:73-81`）与 `gen_audio_tasks()`（`batch/utils/video_processor.py:182-184`） |
