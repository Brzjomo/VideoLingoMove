...
### 5.3 `resolution == '0x0'`（「烧录字幕（压进成片）」关闭）与音频输入

`resolution` 为 `0x0` 等价于侧边栏的「烧录字幕（压进成片）」开关（2026-09-20 前叫 `Burn-in Subtitles`）处于关闭状态。此时**不再生成任何占位视频**（OpenCV 造 1 帧黑视频的旧实现已删）：
...
配套逻辑：UI 用 `os.path.exists(STAGE_DONE_MARKER)`（`st.py`）判断阶段完成；`st_components/sidebar_setting.py` 的「烧录字幕（压进成片）」开关通过把 `resolution` 写成 `'0x0'` / 具体值来表达；批处理同样在 `batch/utils/video_processor.py` 跳过 step7。
