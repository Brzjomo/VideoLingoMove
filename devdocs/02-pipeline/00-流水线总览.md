...
│            → step5 字幕长度切分 → step6 时间轴对齐 + 生成 SRT              │
│            → step7 压制字幕到视频                                          │
└────────────────────────────────────────────────────────────────────────────┘
                    ↓ 产出 output/output_sub.mp4（UI 以此判断阶段完成）
```
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `core/step1_ytdlp.py` | `download_video_ytdlp()` | 用户 URL / 上传文件 | `output/<title>.<ext>`、缩略图 | 无（由 UI 判断是否已有视频） | [`01-下载与音频提取.md`](01-下载与音频提取.md) |
| 2 | `core/step2_whisperX.py` | `transcribe()` | `output/<视频>` | `output/log/cleaned_chunks.xlsx`、`output/audio/*` | `cleaned_chunks.xlsx` 存在 | [`02-语音识别ASR.md`](02-语音识别ASR.md) |
...
| 3.2 | `core/step3_2_splitbymeaning.py` | `split_sentences_by_meaning()` | `sentence_splitbynlp.txt` | `output/log/sentence_splitbymeaning.txt` | 无 | [`03-句子切分NLP.md`](03-句子切分NLP.md) |
...
| 6 | `core/step6_generate_final_timeline.py` | `align_timestamp_main()` | `cleaned_chunks.xlsx`、`translation_results_for_subtitles.xlsx` | `output/{src,trans,src_trans,trans_src}.srt`、`output/cost.txt` | 无 | [`05-字幕切分与时间轴.md`](05-字幕切分与时间轴.md) |
| 7 | `core/step7_merge_sub_to_vid.py` | `merge_subtitles_to_video()` | 视频、`output/src.srt`、`output/trans.srt` | `output/output_sub.mp4` | 无（UI 用此文件判断阶段完成） | [`06-字幕压制与成片.md`](06-字幕压制与成片.md) |
...
| `find_video_files()` | `step1_ytdlp.py` | `step2`、`step7`、`onekeycleanup` |

---

## 五、扩展点（新增/删除/替换步骤）

1. **中间插入一步**：命名 `core/stepN_M_xxx.py`，读上游产物、写新产物，然后在 `st.py: process_text()` 里插入调用，并在 `st_components/imports_and_utils.py` 的 import 列表里加上模块。完整清单见 [`../05-guides/03-如何新增一个流水线步骤.md`](../05-guides/03-如何新增一个流水线步骤.md)。
...
> 建议先设 `$env:PYTHONIOENCODING='utf-8'`，避免 GBK 控制台打印 emoji 崩溃。
