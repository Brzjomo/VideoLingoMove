...
1. **中间插入一步**：命名 `core/stepN_M_xxx.py`，读上游产物、写新产物，然后在 `st.py: build_task_steps()`（`st.py:69`）里插入调用，并在 `st_components/imports_and_utils.py` 的 import 列表里加上模块。完整清单见 [`../05-guides/03-如何新增一个流水线步骤.md`](../05-guides/03-如何新增一个流水线步骤.md)。
