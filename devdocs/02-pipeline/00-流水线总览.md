...
| 5.2 | `core/step5_2_polish_subs.py` | `polish_subs_main` | `translation_results_for_subtitles.xlsx` | `output/log/translation_results_polished.xlsx` | **默认关**：`subtitle.polish_translation` 为假时只打印一行并返回（零 LLM 调用）；仅转录模式也跳过 | [`05-字幕切分与时间轴.md`](05-字幕切分与时间轴.md) |
