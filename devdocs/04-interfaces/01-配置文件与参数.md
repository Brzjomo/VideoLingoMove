...
关键点：**读侧每天在跑，写侧只有 4 个来源**——Streamlit 侧边栏（`st_components/sidebar_setting.py`）、`st.py` 的字幕长度面板（`st.py`，写 `max_split_length` 与 `subtitle.max_length`）、批处理的语言临时改写（`batch/utils/batch_processor.py`）、运行时的自动写回（`whisper.detected_language`，由 `core/all_whisper_methods/whisperX_utils.py` 的 `save_language` 在 写入）。
...
| `model_input` | | 模型选择控件：装了可选依赖 `streamlit-searchbox` 时用搜索框（`key="api_model_searchbox"`，选中即 `update_key("api.model", ...)`），未装则回退 `config_input("MODEL", "api.model")` |
...
| `target_language` | str | `'简体中文'` (:31) | 目标语言，**直接写进提示词**（自然语言即可） | `core/prompts_storage.py`| 是（sidebar_setting.py "Target Lang"） |
...
| `asr_engine` | str | `'whisper'` (:37) | ASR 引擎：`'whisper'` / `'volcano'` | `core/step2_whisperX.py`；sidebar_setting.py | 是（sidebar_setting.py "ASR Engine"） |
...
| `resolution` | str | `'0x0'` (:97) | 压制分辨率；`'0x0'` 表示**不压制** | `st.py`；`core/step7_merge_sub_to_vid.py`；`batch/utils/video_processor.py` | 是（:165 "Burn-in Subtitles" 开关 + :173 分辨率下拉） |
...
| `subtitle.max_length` | int | `75` (:107) | 单行字幕最大长度（字符，加权前） | `core/step5_splitforsub.py`；`st.py`（字幕长度面板也会写） | 是（`st.py` 面板） |
| `subtitle.target_multiplier` | float | `1.2` (:109) | 译文字幕长度折算系数 | `core/step5_splitforsub.py` | 否（`# *`） |
| `subtitle.align_on_mismatch` | str | `warn` (:111) | 字幕对齐兜底策略：`warn`=模糊兜底并告警；`strict`=精确匹配失败即报错 | `core/step6_generate_final_timeline.py`（`warn` 时的提示在） | 否 |
...
| `max_split_length` | int | `20` (:116) | 首次粗切的最大词数（spaCy token 数） | `core/step3_2_splitbymeaning.py`；`st.py`（字幕长度面板也会写） | 是（`st.py` 面板） |
...
| `resolution: '0x0'` | 等价于侧边栏 **Burn-in Subtitles 关闭**：`step7` 静默跳过压制——真实视频**不产出成片**（并删掉遗留的 `output_sub.mp4`）；纯音频输入（`input_manifest.json` 记为 `audio`）本来就只交字幕文件；旧产物 `black_screen.mp4` 走兼容分支被原样保留为成片。要出成片就设为 `'1920x1080'` 或 `'640x360'`（那个开关就是在 `'0x0'` 与分辨率之间切换） | `core/step7_merge_sub_to_vid.py`、`st_components/sidebar_setting.py` |
...
`config.yaml` 的 `version: "2.1.2"` 没有任何代码读取，也没有「旧配置升级」逻辑。因此从上游拉取新代码后，若 `config.yaml` 缺失新键，第一个用到它的 `load_key` 就会 `KeyError`。**新增配置项必须同时修改代码根目录的 `config.yaml`，并且需要手工同步到用户已有的配置文件**（或用外部脚本 merge）。
