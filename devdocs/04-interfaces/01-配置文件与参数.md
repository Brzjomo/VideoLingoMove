...
flowchart TD
 ...
 S52["core/step5_2_polish_subs.py"]
 ...
 BP --> UK
 WX --> UK
 LK --> AG & PR & S2 & S32 & S41 & S42 & S5 & S52 & S67 & SP & S1
 LKO --> S2
 GSL --> PR & S32 & S5 & SP
 GJ --> S32 & S5 & SP
```
...
subtitle: {auto_length_by_language, max_length, target_multiplier, align_on_mismatch,
 align_validate, align_allow_rewrite, polish_translation, polish_thinking,
 polish_long_lines_only, boundary_window, merge_short_cues,
...
| `subtitle.polish_translation` | bool | `false` | **2026-09-21 新增，默认关**。step5.2 字幕润色（译后顺句）：把 step5 切好的逐条字幕译文交给 LLM**只改措辞**（语序/连接词/语气助词），不许增删信息、不许丢数字、不许比原行更长。成本 = **每 20 行一次** `ask_gpt(log_title='polish_subs')` + **对确实改动过的行每个批次一次** `ask_gpt(log_title='polish_audit')` 审校（关着时零调用；仅转录模式自动跳过；**只有润色的分区随思考开关变成 `polish_subs_nothink`，审校恒为 `polish_audit`**）。护栏 `core/subtitle_split.polish_ok` 拦下的行**保持原译文**、审校判为"改了信息"的行**回退原译文**；产物 `output/log/translation_results_polished.xlsx`（行数与 step5 产物恒等）。⚠️ **缺键/读失败一律按关**（与 `align_allow_rewrite` 缺省视为开相反：老 config 不能因为读键失败就偷偷调 LLM） | `core/config_utils.polish_translation`（判定点）→ `core/step5_2_polish_subs.py::polish_enabled`；读取产物的是 `core/step6_generate_final_timeline.py::pick_translation_file` | 是（侧边栏「✨ 字幕润色」里的 toggle「翻译后润色字幕措辞」，改动即写回并 `st.rerun(scope="app")`） |
| `subtitle.polish_thinking` | bool | `true` | **2026-09-21 新增，默认开**。润色时是否允许模型**思考**（判定点 `core/config_utils.polish_thinking`，**缺键/读失败按开** —— 老 config 保持"更忠实但贵"的行为）。关掉时 `core/step5_2_polish_subs.py::polish_lines` 给 `ask_gpt` 传 `extra_body={"thinking": {"type": "disabled"}}`（实测 20 行/批：开 ≈ 20,600 tokens → 关 1,788，**−91%**，这组数字只计润色调用；审校不随开关变、约 860 tokens/次，见 [`../03-subsystems/01-LLM调用与提示词.md`](../03-subsystems/01-LLM调用与提示词.md) §5.1），**并自动把覆盖率门槛从 `subtitle_split.POLISH_COVERAGE_MIN = 0.70` 收紧到 `POLISH_COVERAGE_MIN_NO_THINKING = 0.85`** —— 关思考的模型会明显压缩、丢词（`自由职业手办原型师` → `自由手办原型师`），0.70 拦不住，代价是护栏拦下的行更多（那些行保持原译文）。**`polish_audit` 审校调用不带这个参数**：`_audit_info_changes(pairs, stats)` **没有 `extra_body` 形参**，恒按模型默认档（思考开）跑 —— 用户 2026-09-21 的规则是"思考开关只管润色，其他包括审校默认都开思考"，docstring 写明理由：审校是这一步的安全网（兜住机械护栏拦不住的"换词式增补"），默认档更可靠，且它很便宜（实测一次 3 行审校 ≈ 860 tokens，相对润色那一次的 1,788~20,600 可以忽略）；因此 `_log_title(base, extra_body)` 只给润色调用取名，审校恒落 `polish_audit` 分区，所以**切换这个开关立即生效**，不必删 json | `core/config_utils.polish_thinking`（判定点）→ `core/step5_2_polish_subs.py::polish_lines`（`min_coverage` 也在这里按档位选） | 是（侧边栏「✨ 字幕润色」面板的第二个 toggle「允许模型思考」，润色关闭时置灰；改动即 `update_key` + `st.rerun(scope="app")`） |
| `subtitle.polish_long_lines_only` | bool | `false` | **2026-09-21 新增，默认关**。只润色"有分句的长行"（判定点 `core/config_utils.polish_long_lines_only`，缺键按关 = 全部行都润色）：`polish_lines` 先用 `core/subtitle_split.needs_polish_long_line` 预筛（宽度 ≥ `POLISH_LONG_LINE_MIN_WIDTH = 15.0` ≈ 8~9 个汉字 **且**含 `，、；：,;:` 之一），其余行**原样保留**、只把送出的行数记进 `stats['sent']`（跳过的记 `stats['skipped']`）。实测某 79 行片子可跳过 24 行、批数 4 → 3；与关思考同时打开时全片润色开销 ≈ +5%（全量 + 开思考 ≈ +30%） | `core/config_utils.polish_long_lines_only`（判定点）→ `core/step5_2_polish_subs.py::polish_lines` 的预筛；判据是纯函数 `core/subtitle_split.needs_polish_long_line` | 是（侧边栏同一面板的第三个 toggle「只润色有分句的长行」，润色关闭时置灰） |
