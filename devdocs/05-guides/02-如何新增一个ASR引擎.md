...
> [`06-上游3.0.4合并记录.md`](06-上游3.0.4合并记录.md) 第七节）。新增引擎时要改的是
...
`core/step2_whisperX.py:312` 的 `transcribe_audio()` 是唯一分发点（下列为真实结构，配置不完整时会回退到 whisper）：
...
Whisper 分支是 `core/step2_whisperX.py:239`（`save_language(result['language'])`），火山分支是
`core/step2_whisperX.py:302`（`save_language(result.get('language'))`）。请一并提供该键。

`save_language()` 本体在 `core/all_whisper_methods/whisperX_utils.py:332`：只在拿到
非空且不等于 `'auto'` 的字符串时才写入 `whisper.detected_language`。**不要为了"有值可写"而伪造语言**
（例如自动检测失败时兜底 `'en'`）——`core/config_utils.py:143` 的 `get_source_language()` 会把它当成真实源语言，
让提示词与 spaCy 模型全部用错语种。

### 1.3 缓存身份契约（新增，容易漏）

`transcribe()` 会按内容寻址缓存每个分段的结果（`core/step2_whisperX.py:508-521`，缓存键由
`_asr_cache_settings()` 构造，`core/step2_whisperX.py:408`）。新增引擎时**必须**把"会实质改变识别结果"
的参数加进这个字典，否则换了参数仍会命中旧缓存、表现为"改了参数没效果"。

| 现状 | 位置 |
| --- | --- |
| 公共字段：`asr_engine` / `model` / `language` / `demucs` | `core/step2_whisperX.py:415-420` |
| 引擎私有字段：仅当 `asr_engine == "volcano"` 时追加 `volcano` 子字典（`resource_id`、`language`、`enable_punc`、`enable_itn`、`enable_ddc`、`enable_speaker_info`、`show_utterances`、`enable_channel_split`、`vad_segment`、`model_version`） | `core/step2_whisperX.py:421-430` |
| 缓存目录 `.cache/asr/<key>/<part>.json`，`part = f"{start:.2f}_{end:.2f}"` | `core/all_whisper_methods/transcription_cache.py:29`、`core/step2_whisperX.py:509` |

新增引擎请照火山分支的写法加一个同构的 `if settings["asr_engine"] == "my_asr":` 段。
注意 `cache_key()` 还会自动带上 `whisperx` / `faster-whisper` / `demucs` 的包版本
（`transcription_cache.py:34-42`）与源媒体 md5，这些无需你手动加。
...
| 边界 | 参考现有实现 |
| --- | --- |
| 音频格式要求 | 火山要求 16 kHz / 单声道 / 16-bit PCM WAV，为此有 `convert_video_to_audio()` 里的 `raw.wav` 分支与常量 `RAW_AUDIO_WAV_FILE`（`core/all_whisper_methods/whisperX_utils.py:10`、`:52`） |
| 文件大小限制 | Whisper 有 25 MB 限制，为此有 `split_audio()` 分段（`core/all_whisper_methods/whisperX_utils.py:145`） |
| 鉴权失败 | 抛明确异常，不要静默返回空结果（火山分支见 `core/all_whisper_methods/volcano_asr.py:372` 的 `transcribe_audio()`） |
| 时间戳偏移 | 若你切了片，**必须把结果的时间戳加回 `start` 偏移**。Whisper 分支见 `core/step2_whisperX.py:245-260`；火山分支见 `core/all_whisper_methods/volcano_asr.py:400-404` 的 `result_start_offset` |
| 长循环可取消 | 分段循环已经由 `transcribe()` 里的 `eu.check_cancel()` 覆盖（`core/step2_whisperX.py:500`），但**引擎内部若有自己的长循环/轮询等待**（如上传 TOS 后轮询任务状态），也应在循环里调一次 `eu.check_cancel()`，否则"停止"要等整段跑完 |
| Windows 控制台 | 若新模块带 `__main__` 且会打印 emoji，照现有 18 个模块的写法在入口调 `easy_util.ensure_utf8_console()`（示例见 `core/step2_whisperX.py:540-548`），否则 `python -m core.all_whisper_methods.my_asr` 会抛 `UnicodeEncodeError` |
...
并在 `transcribe_audio()` 中增加分支，同时处理「配置不完整时回退 whisper」的逻辑（现有 volcano 分支的做法见 `core/step2_whisperX.py:329-337`，推荐照抄这个模式）。

### 步骤 3：准备输入音频

`transcribe()` 里有两条音频准备路径（`core/step2_whisperX.py:472-490`）：
...
（`core/all_whisper_methods/whisperX_utils.py:52-94`）——不要在新增分支里把它们改成从彼此转码。
...
注意 `asr_engine` 在模板里的位置是 `config.example.yaml:37`（**不是** `config.yaml:36`）。
若你把合法取值写进注释，**改代码时同步更新这行注释与 `st_components/sidebar_setting.py:192-195` 的
`asr_engines` 字典**（两处都要改，否则 UI 无法选中新引擎）。

### 步骤 5：接入转录缓存

照 1.3 的写法在 `_asr_cache_settings()`（`core/step2_whisperX.py:408`）里为新引擎补一段，
...
| 整体回归 | `python -m unittest discover -s tests -v`（46 例，标准库 unittest，不需 pytest） |

清理手段：UI 侧边栏底部的「♻️ 转录缓存」折叠区有「清空转录缓存」按钮
（`st.py:251` 的 `cache_maintenance_section()` → `transcription_cache.clear_cache()`）。
