...
| **点名单个模型** | `--only hf_own:<模型名>` / `--only torch_ckpt:<权重名>`：报告里列出的具体模型可以直接点名删，不必用 `--models` 一次清掉全部本项目模型。名字照抄报告里的 key（`build_targets()` 里打印的 `hf_own:` / `torch_ckpt:` 前缀）。模型不存在时 `select_targets()` 选不中，`clean()` 会打印"没有匹配的清理目标"并返回 1 |
