...
| `OneKeyStart.bat` | 93 | 一键启动：纯 ASCII + `chcp 65001` + `cd /d "%~dp0"` + **解释器探测**（`.venv` → conda → PATH）+ `installer.py --check --quiet` + `launch.py` |
| `batch/StartBatch.bat` | 77 | 批处理界面：`cd /d "%~dp0.."` + 解释器探测 + `-m streamlit run "batch\utils\gui.py"` |
| `AudioExtract/Start.bat` | 70 | 音频提取工具：`cd /d "%~dp0"` + 解释器探测 + `-m streamlit run gui.py` |
...
文件共 94 行：**31 条有效项** + 55 行注释 + 8 行空行（`:19`、`:36`、`:45`、`:56`、`:68`、`:78`、`:83`、`:86`）。
...
| 67 | `requests==2.32.3` | 锁定 | `core/all_whisper_methods/volcano_asr.py:11`、`core/ask_gpt.py`、`core/pypi_autochoose.py:3`、`st_components/sidebar_setting.py:32` | ✅ |
| 70 | `pandas>=2.2.3,<3.0.0` | 区间 | step4/5/6 的表格读写、`core/all_whisper_methods/whisperX_utils.py:2`、`batch/utils/*` | ✅ |
...
| 74 | `ruamel.yaml>=0.18.0,<0.19.0` | 区间 | `core/config_utils.py:1`，保留引号/注释地读写 `config.yaml` | ✅ |
| 75 | `json-repair>=0.30.0,<0.50.0` | 区间 | `core/ask_gpt.py:4`（PyPI 包名 `json-repair`，import 名 `json_repair`） | ✅ |
| 76 | `yt-dlp` | **无约束（每次安装取最新）** | `core/step1_ytdlp.py:101`；另外 `:95` 在每次下载前还会 `pip install --upgrade yt-dlp` | ✅ |
| 77 | `autocorrect-py` | 无约束 | `core/step6_generate_final_timeline.py:11` 的 `import autocorrect_py as autocorrect` | ✅ |
| 80 | `syllables` | 无约束 | `core/estimate_duration.py:1`，按音节数估算朗读时长（被 `core/subtitle_trim.py` 使用） | ✅ |
| 81 | `pypinyin` | 无约束 | `core/estimate_duration.py:2`，中文拼音 | ✅ |
| 82 | `g2p-en` | 无约束 | `core/estimate_duration.py:3`，英文 G2P | ✅ |
| 85 | `tos>=2.8.7` | 仅下限 | `core/all_whisper_methods/tos_service.py:83` 的 `import tos`（火山引擎对象存储） | ✅ |
