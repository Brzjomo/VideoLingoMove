...
**结论（已读代码确认）：用 `subprocess.run` 调外部 `ffmpeg` 可执行文件，未使用 moviepy / moviepy.editor / pydub / imageio-ffmpeg。** 全仓库对 `moviepy` 的引用只出现在 `requirements.txt:5`，`AudioExtract/` 下没有任何 moviepy import。
...
4. **`-acodec libmp3lame` 要求 ffmpeg 编译了 libmp3lame**。本机根目录的 `ffmpeg.exe` 确认含该编码器（configure 参数里 `--enable-libmp3lame`），但**该 ffmpeg 需要 PATH 注入**（`st.py:8-9` 只对主应用做这件事，`AudioExtract/Start.bat` 不做）——如果 `ffmpeg` 不在 PATH，每个文件都会各抛一次 `FileNotFoundError` 并被吞成「✗ 提取失败」，界面只会显示「失败 N 个文件」，没有任何「找不到 ffmpeg」的提示。
...
10. **`st.button(..., use_container_width=True)`**（`gui.py:106`、`:113`、`:130-132`）在 `streamlit==1.38.0`（`requirements.txt:17`）下仍可用；Streamlit 1.4x 起该参数被 `width` 取代并告警——升级 Streamlit 时要同步改这三处。
