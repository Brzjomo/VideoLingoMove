@echo off
rem ============================================================================
rem  VideoLingo batch mode (Windows) -- launches batch\utils\gui.py
rem
rem  Interpreter order: project .venv  ->  conda env videolingo  ->  python on PATH
rem
rem  Fixes over the old version:
rem    * the old script did "cd .." which lands in the PARENT of the project when
rem      invoked from the project root; now it resolves its own directory;
...
if exist "%VENV_PY%" (
    "%VENV_PY%" -c "import sys; raise SystemExit(0 if (3,10)<=sys.version_info[:2]<(3,14)and __import__('streamlit')and __import__('pandas')else 1)" >nul 2>&1
    if not errorlevel 1 (
        set "PY=%VENV_PY%"
...
if exist "%CONDA_ENV%\python.exe" (
    "%CONDA_ENV%\python.exe" -c "import sys; raise SystemExit(0 if (3,10)<=sys.version_info[:2]<(3,14)and __import__('streamlit')and __import__('pandas')else 1)" >nul 2>&1
    if not errorlevel 1 (
...
python -c "import sys; raise SystemExit(0 if (3,10)<=sys.version_info[:2]<(3,14)and __import__('streamlit')and __import__('pandas')else 1)" >nul 2>&1
if not errorlevel 1 (
    set "PY=python"
...
:run
%PY% -c "import sys; raise SystemExit(0 if (3,10)<=sys.version_info[:2]<(3,14)and __import__('streamlit')and __import__('pandas')else 1)" >nul 2>&1
if errorlevel 1 (
...
if errorlevel 1 (
    echo [hint] Health check reported problems -- see above.
    echo.
)

echo [2/2] starting batch mode ...
